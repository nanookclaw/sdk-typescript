...
- **Reading injected config in workflow code (advanced).** The plugin injects
  its configuration into the workflow bundle with webpack's `DefinePlugin` under
  a bare global identifier. If you extend the plugin and need to read an
  injected global in workflow code, read it via **dotted access**
  (`globalThis.__NAME__`), never bracket access (`globalThis["__NAME__"]`) —
  `DefinePlugin` only rewrites the dotted form, and bracket access silently
